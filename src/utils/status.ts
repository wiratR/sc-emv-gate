...
const STALE_PILL_CLASS = "bg-amber-100 text-amber-900 border-amber-200";

/** คืนคลาสสีสำหรับ badge/pill (รองรับ stale) */
export const statusClass = (s: EffectiveStatus): string =>
  s === "stale" ? STALE_PILL_CLASS : BASE_STATUS_CLASSES[s];
...
const STALE_SUMMARY_CLASS = "bg-amber-50 text-amber-900 border-amber-200";
...
export const summaryToneClass = (tone?: EffectiveStatus): string => {
  ...
  return tone === "stale" ? STALE_SUMMARY_CLASS : BASE_SUMMARY_CLASSES[tone];
};

// ── ตัวช่วยอื่น ๆ ──────────────────────────────────────────
/** map 'stale' → 'offline' สำหรับบาง UI ที่ต้องการกลุ่มกว้าง */
export const toDeviceStatusForUi = (s: EffectiveStatus): DeviceStatus =>
  s === "stale" ? "offline" : s;

/** ใช้ตัดสินใจว่าสั่งงานได้ไหม (ตอนนี้อนุญาตเฉพาะ online) */
export const isControllable = (s: EffectiveStatus) => s === "online";

/** เผื่อบางที่ต้องการ helper ง่าย ๆ */
